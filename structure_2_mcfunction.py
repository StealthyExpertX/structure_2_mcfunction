...
def checkAround(x, y, z, level, box, (checkFor, checkForData)):
...
def split_file(filepath, lines_per_file=9999):
	lpf = lines_per_file
...
def convertToData(block, data):
	if block in datablocks.keys():
		if data in datablocks[block].keys():
			return (datablocks[block][data])
	return (level.blockDataAt(x, y, z))
